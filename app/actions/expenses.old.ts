...
async function getSupabaseClient(): Promise<ReturnType<typeof createServerComponentClient<Database>>> {
  try {
...
export async function createExpense(expenseData: ExpenseInput): Promise<{ data: any; error: Error | null }> {
  try {
    ...
    // Prepare the expense data for insertion with proper typing
    const insertData = {
      user_id: user.id,
    ...
    // Insert the expense with proper typing
    const { data, error } = await supabase
      .from("expenses")
      .insert(insertData)
      .select()
    ...
    if (error || !data) {
      ...
      return { data: null, error: error || new Error("Failed to create expense") }
    }

    // Handle category links if provided
    if (expenseData.category_ids?.length && data.id) {
    ...
    if (fetchError) {
      ...
      return { error: new Error("Failed to fetch expense for deletion") }
    }
