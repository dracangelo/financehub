...
-- Only allow admins to modify tax categories
CREATE POLICY "Only admins can modify tax categories" 
  ON public.tax_categories 
  FOR ALL 
...
CREATE TABLE IF NOT EXISTS public.tax_deductions (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE,
  FOREIGN KEY (category_id) REFERENCES tax_categories(id) ON DELETE SET NULL
);

-- Add row level security to tax_deductions
ALTER TABLE public.tax_deductions ENABLE ROW LEVEL SECURITY;

-- Create policy for users to only see their own tax deductions
CREATE POLICY "Users can only see their own tax deductions" 
  ON public.tax_deductions 
  FOR SELECT 
  USING (auth.uid() = user_id);

-- Create policy for users to insert their own tax deductions
CREATE POLICY "Users can insert their own tax deductions" 
  ON public.tax_deductions 
  FOR INSERT 
  WITH CHECK (auth.uid() = user_id);

-- Create policy for users to update their own tax deductions
CREATE POLICY "Users can update their own tax deductions" 
  ON public.tax_deductions 
  FOR UPDATE 
  USING (auth.uid() = user_id);

-- Create policy for users to delete their own tax deductions
...
-- Add row level security to tax_documents
ALTER TABLE public.tax_documents ENABLE ROW LEVEL SECURITY;

-- Create policy for users to only see their own tax documents
CREATE POLICY "Users can only see their own tax documents" 
  ON public.tax_documents 
  FOR SELECT 
  USING (auth.uid() = user_id);

-- Create policy for users to insert their own tax documents
CREATE POLICY "Users can insert their own tax documents" 
  ON public.tax_documents 
  FOR INSERT 
  WITH CHECK (auth.uid() = user_id);

-- Create policy for users to update their own tax documents
CREATE POLICY "Users can update their own tax documents" 
  ON public.tax_documents 
  FOR UPDATE 
  USING (auth.uid() = user_id);

-- Create policy for users to delete their own tax documents
